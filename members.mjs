...
export default async (req, context) => {
  ...
  for (const key of keys) {
    try {
      ...
      // Sessions where at least three sets were actually ticked off.
      const trained = Object.values(rec.sessions || {})
        .filter((s) => Object.keys((s && s.done) || {}).length >= 3).length;

      // Deliberately absent: weight, goal weight, waist, and anything derived
      // from them. An organiser needs to know who is turning up, not what
      // anyone weighs. Body data stays with the person it belongs to.
      members.push({
        id: key,
        name: p.name || "",
        email: rec.email || "",
        ...
        planSource: p.customPlan ? "own plan" : "generated",
